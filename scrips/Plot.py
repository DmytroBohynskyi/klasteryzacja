...
def plot_3d(data, centroids, color, fig_type):
    fig = plt.figure()
    fig.suptitle(f"{fig_type}: cluster = {len(centroids)}", fontsize=16)
...
def plot_k_means(data, centroids, color='gray', end=True, fig_type="None"):
    if data.shape[1] is 2:
        ...
        plt.title(f"{fig_type}: cluster = {len(centroids)}")
    elif data.shape[1] > 2:
        plot_3d(data, centroids, color, fig_type)
